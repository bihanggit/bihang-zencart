...
class Bihang_OAuth
{
    ...
    {
        $url = "https://bihang.com/oauth/authorize?response_type=code" .
    ...
    public function getTokens($code, $grantType='authorization_code')
    {
        ...
        curl_setopt($curl, CURLOPT_URL, 'https://bihang.com/oauth/token');
        curl_setopt($curl, CURLOPT_CAINFO, dirname(__FILE__) . '/ca-bihang.crt');
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array('User-Agent: BihangPHP/v1'));
        ...
        if($response === false) {
            ...
            curl_close($curl);
            throw new Bihang_ConnectionException("Could not get tokens - network error " . $message . " (" . $error . ")");
        }
        if($statusCode !== 200) {
            throw new Bihang_ApiException("Could not get tokens - code " . $statusCode, $statusCode, $response);
        }
        curl_close($curl);
        ...
        try {
            ...
            throw new Bihang_ConnectionException("Could not get tokens - JSON error", $statusCode, $response);
        }
